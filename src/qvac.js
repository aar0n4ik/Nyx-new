...
// We never hardcode a constant the installed SDK doesn't expose — we scan the
// SDK's real exports and fall back gracefully.
...
// Persisted user choice (from the in-app / site model picker), resolved to a
// constant this SDK build actually exposes. Falls through if none chosen.
...
process.env.NYX_QVAC_MODEL_PATH ? `qvac:local-gguf` : `qvac:${pickModelConst()}`
...
// Persist a new model choice and drop the loaded one so the very next request
// loads the newly selected model. No process restart required.
